...
requests["Find all the actors who have not acted in any movie between 1990 and 2000"] = "SELECT id, act_fname, act_lname, mov_title FROM movies WHERE mov_year > 1989;" 
...
requests["Find the actors with all information who played a role in the movies 'Annie Hall'"] = "SELECT actors.* FROM actors INNER JOIN movies_actors ON actors.id = movies_actors.act_id INNER JOIN movies ON movies.id = movies_actors.mov_id WHERE mov_title = 'Annie Hall';"
...
requests["Find the name of movie and director who directed a movies that casted a role as Sean Maguire"] = "SELECT dir_fname, dir_lname, mov_title FROM movies INNER JOIN directors_movies ON movies.id = directors_movies.mov_id INNER JOIN directors ON directors.id = directors_movies.dir_id INNER JOIN movies_actors ON movies_actors.mov_id = movies.id INNER JOIN actors ON actors.id = movies_actors.act_id WHERE role = 'Sean Maguire' ORDER BY mov_title, dir_fname, dir_lname ASC;"
